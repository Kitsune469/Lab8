...
<body>
    ...
    if($result = $mysqli->query('SELECT s.PlayerName, g.Clubname,k.Namecoach FROM players AS s LEFT JOIN futclub AS g ON s.IDfutclub = g.IDfutclub INNER JOIN coach AS k ON g.IDcoach = k.IDcoach ORDER BY s.PlayerName')) {   // $mysqli - наш об'єкт, через який здійснюємо підключення, query - метод, який дозволяє виконати довільний запит

        printf("<h3>Звіт - Список гравців, клубів і тренерів: </h3><br>");   // <br> в html - розрив рядка
        printf("<table><tr><th>ПІБ</th><th>Назва клубу</th><th>Тренер</th></tr>");   // <br> в html - розрив рядка
        ...
        while( $row = $result->fetch_assoc() ){ // fetch_assoc() повертає рядок із запиту у вигляді асоціативного масиву, наприклад $row = ['id'=>'1', 'pib'=>'Олександр', 'grupa'=>'ІПЗ-31']
            printf("<tr><td>%s</td><td>%s</td><td>%s</td></tr>", $row['PlayerName'], $row['Clubname'], $row['Namecoach']); //виводимо результат на сторінку
        };
        printf("</table>");
        /*Звільняємо пам'ять*/
        $result->close();
