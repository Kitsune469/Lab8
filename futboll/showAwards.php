...
<body>
    ...
    if($result = $mysqli->query('SELECT * FROM awards')) {   // $mysqli - наш об'єкт, через який здійснюємо підключення, query - метод, який дозволяє виконати довільний запит
        ...
        printf("Список нагород: <br><br>");   // <br> в html - розрив рядка
        printf("<table><tr><th>ID нагороди</th><th>Назва нагороди</th><th>Дата видачі</th></tr>");   // <br> в html - розрив рядка
        ...
        while( $row = $result->fetch_assoc() ){ // fetch_assoc() повертає рядок із запиту у вигляді асоціативного масиву, наприклад $row = ['id'=>'1', 'pib'=>'Олександр', 'grupa'=>'ІПЗ-31']
            printf("<tr><td>%s</td><td>%s</td><td>%s</td></tr>", $row['IDavard'], $row['Nameavards'], $row['Data']); //виводимо результат на сторінку
        };
        printf("</table>");
        /*Звільняємо пам'ять*/
        $result->close();
